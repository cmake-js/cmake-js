...
const debug = require('debug')('cmake-js:find-visualstudio')
...
class VisualStudioFinder {
	...
	async findNewVSUsingSetupModule(supportedYears) {
		...
		debug('Running', ps, checkModuleArgs)
		...
		if (cErr) {
			...
			debug('VSSetup error = %j', cErr && (cErr.stack || cErr))
			return null
		}
	...
	parseData(err, stdout, stderr, sanityCheckOptions) {
		...
		if (err) {
			debug('PS err = %j', err && (err.stack || err))
	...
	processData(vsInfo, supportedYears) {
		vsInfo = vsInfo.map((info) => {
			debug(`processing installation: "${info.path}"`)
	...
	getVersionInfo(info) {
		...
		if (!match) {
			debug('- failed to parse version:', info.version)
			return {}
		}
		debug('- version match = %j', match)
		const ret = {
		...
		debug('- unsupported version:', ret.versionMajor)
		return {}
	}
	...
	getToolset(info, versionYear) {
		...
		if (process.arch === 'arm64' && info.packages.includes(pkgArm64)) {
			...
			if (process.arch === 'arm64') {
				...
				debug(`- found ${vcToolsX64}`)
			}
		} else if (info.packages.includes(express)) {
			debug('- found Visual Studio Express (looking for toolset)')
		} else {
			return null
		}
		...
		debug('- invalid versionYear:', versionYear)
		return null
	}
	...
	getSDK(info) {
		...
		info.packages.forEach((pkg) => {
			...
			if (parts.length > 5 && parts[5] !== 'Desktop') {
				debug('- ignoring non-Desktop Win10/11SDK:', pkg)
				return
			}
			const foundSdkVer = parseInt(parts[4], 10)
			if (isNaN(foundSdkVer)) {
				// Microsoft.VisualStudio.Component.Windows10SDK.IpOverUsb
				debug('- failed to parse Win10/11SDK number:', pkg)
				return
			}
			debug('- found Win10/11SDK:', foundSdkVer)
