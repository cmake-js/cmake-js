...
BuildSystem.prototype._ensureInstalled = async function () {
    try {
        ...
        if (this.options.isNodeApi) {
            this.options.nodeLibDefPath = await this._generateNodeLibDef(path.join(this.options.out, 'node-lib.def'));
        } else {
...
BuildSystem.prototype._generateNodeLibDef = async function (targetFile) {
    try {
