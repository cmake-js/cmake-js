...
function downloadTo(url, result, calculateSum) {
    ...
    return new Bluebird(function (resolve, reject) {
        request
            ...
            .on('data', function (chunk) { if (shasum) shasum.update(chunk) })
            .pipe(result);
...
function testSum(sums, sum, fPath) {
    var serverSum = _.first(sums.filter(function (s) { return s.path == fPath }));
        ...
        return downloadTo(sumUrl, result, false)
            .then(function () {
                return result.toString()
                    ...
                    .filter(function(i) { return i.path && i.path });
            });
    },
