...
Toolset.prototype.initializeWin = async(function*(install) {
    ...
    this.log.verbose("TOOL", "Looking for make in the path.");
    let sysMake = null;
    ...
    if (sysMake) {
        this.log.info("TOOL", "Make found: " + sysMake);
        try {
            sysMakeVersion = yield this._getVersion(sysMake);
            this.log.info("Make version: " + sysMakeVersion);
        }
        catch (e) {
            this.log.verbose("TOOL", "Cannot determine make version because of error:\n" + e.stack);
            _.noop(e);
        }
    }
    else {
        this.log.verbose("TOOL", "Make is not in the path.");
    }
    ...
    if (!sysCompilerVersion || semver.lt(sysCompilerVersion, compilerDistro.version)) {
        ...
        this.cppCompilerPath = sysCompiler;
        try {
            this.cCompilerPath = yield which("gcc.exe");
        }
        catch(e) {
            _.noop(e);
        }
    ...
    if (this.makePath) {
        this.makePath = this.makePath.replace(/\\/g, "/");
    }
    if (this.cCompilerPath) {
        this.cCompilerPath = this.cCompilerPath.replace(/\\/g, "/");
    }
    if (this.cppCompilerPath) {
        this.cppCompilerPath = this.cppCompilerPath.replace(/\\/g, "/");
    }
});
...
Toolset.prototype._getVersion = async(function*(exePath) {
    this.log.silly("TOOL", "Getting version of " + exePath);
    let exists;
    try {
    ...
    if (!exists) {
        this.log.silly("TOOL", "Doesn't exists.");
        return null;
    }
    try {
        ...
        if (stdout) {
            ...
            if (firstLine) {
                this.log.silly("TOOL", "Version output: " + firstLine);
            ...
            this.log.silly("TOOL", "Stdout is empty.");
        }
    }
    catch (e) {
        this.log.silly("TOOL", "Cannot determine the version because of error:\n" + e.stack);
        _.noop(e);
    }
    return null;
});
