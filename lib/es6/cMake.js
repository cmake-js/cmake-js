...
CMake.prototype.getConfigureCommand = async(function* (options) {
    // Create command:
    ...
    // Custom options
    let o = options || {};
    for (let k of _.keys(o)) {
      D.push({ [k.toUpperCase()]: o[k] });
    }
...
CMake.prototype.ensureConfigured = async(function* (options) {
    try {
        yield fs.lstatAsync(path.join(this.workDir, "CMakeCache.txt"));
    }
    catch (e) {
        _.noop(e);
        yield this.configure(options);
    }
});
...
CMake.prototype.build = async(function* (options) {
    this.verifyIfAvailable();

    yield this.ensureConfigured(options);
...
CMake.prototype.reconfigure = async(function* (options) {
    yield this.clean();
    yield this.configure(options);
});

CMake.prototype.rebuild = async(function* (options) {
    yield this.clean();
    yield this.build(options);
});

CMake.prototype.compile = async(function* (options) {
    try {
        yield this.build(options);
    }
    catch (e) {
