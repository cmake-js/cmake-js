...
function getConfig(lookPath, log) {
    ...
    try {
        ...
        if (_.isPlainObject(json) && _.isPlainObject(json.config) && _.isPlainObject(json.config["cmake-js"])) {
            log.silly("CFG", "Config found.");
            return null;//json.config["cmake-js"];
        }
        else {
...
module.exports = function (projectPath, log) {
    ...
    for (; ;) {
        ...
        try {
            ...
            currPath = path.normalize(path.join(currPath, ".."));//findParentDir.sync(path.normalize(path.join(currPath, "..")), "tmp");
