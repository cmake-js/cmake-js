...
CMake.prototype.getConfigureCommand = function () {
    ...
    return init.then(function () {
        ...
        try {
            ...
            if (nanH) {
                nanH = path.resolve(path.join(self.projectRoot, nanH));
                incPaths.push(nanH);
                console.log("NAN: " + nanH);
            }
        }
        catch (e) {
            _.noop(e);
        ...
        if (environment.isOSX) {
            D.push({ "CMAKE_CXX_FLAGS": "-D_DARWIN_USE_64_BIT_INODE=1 -D_LARGEFILE_SOURCE -D_FILE_OFFSET_BITS=64 -DBUILDING_NODE_EXTENSION -w" });
