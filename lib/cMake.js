...
CMake.prototype.build = function (noRetry) {
    this.verifyIfAvailable();
    ...
    return self.ensureConfigured()
        ...
        .then(function (buildCommand) {
            ...
            return self._run(buildCommand)
                .catch(function(e) {
                    if (retry) {
                        self.log.error("OMG", e.stack);
                        self.log.error("OMG", "Build has been failed. Trying a full REBUILD.");
...
CMake.prototype.rebuild = function (noRetry) {
    var self = this;
    return self.clean()
        .then(function () {
            return self.build(noRetry);
        });
};
