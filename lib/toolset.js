...
const findVisualStudioRaw = require('./import/find-visualstudio');
const { generateNodeLib } = require("./nodeApi");
    ...
    if (!this.generator) {
        ...
        if (foundVsInfo) {
            ...
            if (this.options.isNodeApi) {
                // Generate the node.lib file
                this.winNodeLibPath = await generateNodeLib(this.targetOptions, foundVsInfo, path.join(this.options.out, 'node.lib'))
            }
        } else {
