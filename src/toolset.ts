...
import { findVisualStudio as findVisualStudioRaw } from "./import/find-visualstudio";
import { CMake } from "./cMake";

const findVisualStudio = util.promisify(findVisualStudioRaw);

export interface ToolsetOptions extends CMLogOptions, TargetOptionsOptions {
  generator?: string;
  toolset?: string;
  platform?: string;
  target?: string;
...
export class Toolset {
  ...
  readonly msvsVersion?: string;
  readonly preferClang?: boolean;
  readonly preferGnu?: boolean;
  readonly preferXcode?: boolean;
  readonly preferMake?: boolean;
  readonly compilerFlags: string[];
  readonly linkerFlags: string[];
  readonly makePath: string | null;
  ...
  constructor(protected options: ToolsetOptions = {}) {
  ...
  async initialize(install: boolean = false) {
  ...
  initializePosix(install: any) {
    if (!this.cCompilerPath || !this.cppCompilerPath) {
      ...
      if (this.options.preferClang && environment.isClangAvailable) {
        if (install) {
          this.log.info(
            "TOOL",
            "Using clang++ compiler, because preferClang option is set, and clang++ is available."
          );
        }
        ...
        if (install) {
          this.log.info(
            "TOOL",
            "Using g++ compiler, because preferGnu option is set, and g++ is available."
          );
        }
    ...
    if (this.generator) {
      if (install) {
        this.log.info(
          ...
          "Using " +
            this.generator +
            " generator, as specified from commandline."
        );
      }
    }
    // 2: Generator
      ...
      if (this.options.preferXcode) {
        if (install) {
          this.log.info(
            "TOOL",
            "Using Xcode generator, because preferXcode option is set."
          );
        }
        ...
        if (install) {
          this.log.info(
            "TOOL",
            "Using Ninja generator, because ninja is available."
          );
        }
    ...
    if (this.options.target) {
      this.log.info(
        ...
        "Building only the " +
          this.options.target +
          " target, as specified from the command line."
      );
    }
  }

  async initializeWin(install: boolean) {
    if (!this.generator) {
      ...
      if (foundVsInfo) {
        ...
        if (!foundVsInfo.versionMajor) return;
        ...
        if (!this.platform && isAboveVS16) {
          switch (this.targetOptions.arch) {
            ...
            default:
              this.log.warn(
                "TOOL",
                "Unknown NodeJS architecture: " + this.targetOptions.arch
              );
              break;
          }
        }
  ...
  async _getTopSupportedVisualStudioGenerator() {
    ...
    const selectedVs = await findVisualStudio(this.options.msvsVersion!);
    ...
    for (const gen of list) {
      ...
      if (!selectedVs.versionMajor) continue;
      ...
      if (!isAboveVS16) {
        ...
        if (
          (this.targetOptions.isX86 && is64Bit) ||
          (this.targetOptions.isX64 && !is64Bit)
        ) {
          continue;
        }
      }

      return { ...selectedVs, generator: gen };
    }
