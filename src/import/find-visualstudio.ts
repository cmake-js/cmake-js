...
import { logWithPrefix, regSearchKeys } from "./util";
...
export type VSFinderCallback = (
  error: Error | null,
  info?: VSInfo | null
) => void;

class VisualStudioFinder {
  protected readonly errorLog: string[] = [];
  protected readonly validVersions: (string | number)[] = [];
  protected readonly regSearchKeys = regSearchKeys;
  protected readonly log: ReturnType<typeof logWithPrefix>;

  protected configVersionYear: number | null = null;
  protected configPath: string | null = null;
  protected envVcInstallDir: string | null = null;
  protected nodeSemver: SemVer | null = null;

  constructor(
    protected readonly configMsvsVersion: string,
    protected readonly callback: VSFinderCallback
  ) {
    this.log = logWithPrefix(npmlog, "find VS");
  }
  ...
  findVisualStudio2017OrNewer(cb: (info: VSInfo | null) => void) {
    const ps = path.join(
  ...
  parseData(
    ...
    this.log.silly("vsInfo:", returns);

    // Remove future versions or errors parsing version number
    const filteredReturns = returns.filter((info) => {
      if (info.versionYear) {
        return true;
      }
    ...
    filteredReturns.sort((a, b) => (b.versionYear || 0) - (a.versionYear || 0));
  ...
  getVersionInfo(info: VSInfo): Return {
  ...
  getMSBuild(info: VSInfo, versionYear?: number | null) {
  ...
  getToolset(info: VSInfo, versionYear?: number | null) {
  ...
  findVisualStudio2015(cb: (info: VSInfo | null) => void) {
    if (this.nodeSemver?.major && this.nodeSemver?.major >= 19) {
      this.addLog(
  ...
  findOldVS(info: VSInfo, cb: (info: VSInfo | null) => void) {
    const regVC7 = [
  ...
  checkConfigVersion(versionYear: number, vsPath: string) {
    ...
    if (
      this.configPath &&
      path.relative(this.configPath, vsPath.toString()) !== ""
    ) {
...
export const findVisualStudio = (
  configMsvsVersion: string,
  callback: VSFinderCallback
) => {
  const finder = new VisualStudioFinder(configMsvsVersion, callback);
